...
- **코드/파일 참조 전송** — 에디터에서 코드나 파일을 참조하거나, 프로젝트 뷰에서 파일/디렉터리를 선택해 실행 중인 Codex 터미널에 `@파일경로#L줄번호`, `@파일경로#L시작줄-끝줄`, `@파일경로` 형식의 참조 전송
...
텍스트를 선택하지 않은 경우에는 현재 파일이 `@파일경로` 형식으로 전송됩니다.

### 파일/디렉터리 참조 전송

1. 프로젝트 뷰에서 파일 또는 디렉터리 선택
2. 우클릭 메뉴 → **Copy to Codex Terminal**
3. 선택한 항목의 참조가 `@파일경로` 형식으로 실행 중인 Codex 터미널로 전송됨

여러 항목을 선택한 경우에는 `@파일경로 @파일경로`처럼 공백으로 구분된 참조가 함께 전송됩니다.

> Codex가 실행 중이지 않은 경우 참조 전송 액션은 비활성화됩니다.
